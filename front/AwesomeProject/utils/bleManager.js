...
const SERVICE_UUID = '1E3D3700-1111-2222-3333-444444444444';
const CHARACTERISTIC_UUID = '1E3D3701-1111-2222-3333-444444444444';
...
class LOCBleManager {
  ...
  // Send settings configuration payload to paired device
  async syncSettings(settings) {
    // Drop UI/device metadata before writing settings data to hardware
    const { device, ...syncPayload } = settings;
    const settingsJson = JSON.stringify(syncPayload);

    if (isVirtualMode) {
      console.log('[BLE Sync] Virtual transmitting settings configuration...');
      console.log('[BLE Sync] Payload String:', settingsJson);
      
      const base64Payload = utf8ToBase64(settingsJson);
      console.log('[BLE Sync] Payload Base64:', base64Payload);
      ...
      console.log('[BLE Sync] Transmitting settings payload over physical BLE...');
      const base64Payload = utf8ToBase64(settingsJson);
